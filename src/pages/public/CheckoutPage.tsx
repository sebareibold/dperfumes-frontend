...
export default function CheckoutPage() {
  ...
  const shouldHaveFreeShipping =
    ...
    subtotal >=
      (checkoutContent.shipping.homeDelivery.freeShippingThreshold ?? 0);

  const shippingCost =
    wantsShipping && !shouldHaveFreeShipping
      ? checkoutContent?.shipping?.homeDelivery?.baseCost ?? 2500
      : 0;
  ...
  const loadCheckoutContent = async (forceRefresh = false) => {
    try {
      ...
      if (response.success && response.content?.checkout) {
        ...
        console.warn(
          "⚠️ No se encontró contenido de checkout, usando valores por defecto"
        );
  ...
  const handleCheckout = async () => {
    ...
    try {
      if (paymentMethod === "transfer" && transferProof) {
        const uploadResult = await apiService.uploadTransferProof(
          transferProof
        );
        if (uploadResult.success) {
          ...
          throw new Error(
            uploadResult.error ||
              "Error al subir el comprobante de transferencia"
          );
        }
      }
      ...
      let errorMessage =
        "Error procesando tu orden. Por favor intenta nuevamente.";

      if (error instanceof Error) {
        if (error.message.includes("400")) {
          errorMessage =
            "Hay un problema con los datos del pedido. Verifica la información e intenta nuevamente.";
        } else if (error.message.includes("500")) {
          errorMessage =
            "Error interno del servidor. Por favor intenta más tarde o contacta soporte.";
      ...
      >
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-16">
          <div className="text-center py-12 sm:py-20 bg-white rounded-2xl shadow-warm">
            <h2
              className="font-serif text-xl sm:text-2xl font-light mb-4"
              style={{ color: "var(--deep-clay)" }}
            >
              Tu carrito está vacío
            </h2>
            <p
              className="text-base sm:text-lg font-light mb-6 sm:mb-8"
              style={{ color: "var(--oak)" }}
            >
    ...
    >
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-16 ">
        ...
        <div className="mb-8 sm:mb-12">
          ...
          <h1
            className="font-serif text-2xl sm:text-4xl lg:text-5xl font-light"
            style={{ color: "var(--deep-clay)" }}
          >
            Finalizar Compra
          </h1>
        ...
        <div className="lg:grid lg:grid-cols-12 lg:gap-12 space-y-8 lg:space-y-0 ">
          ...
          <div className="lg:col-span-8">
            <div className="bg-white rounded-2xl shadow-warm shadow-lg shadow-2xl p-4 sm:p-6 lg:p-8 ">
                      ...
                      <div className="flex items-start sm:items-center">
                        <MapPin
                          className={`h-5 w-5 sm:h-6 sm:w-6 mr-2 sm:mr-3 flex-shrink-0 mt-1 sm:mt-0 ${
                            wantsShipping ? "text-black" : "text-white"
                          }`}
                        />
                        <div>
                          <h3
                            ...
                            style={{
                              color: !wantsShipping ? "#ffffff" : "#111",
                            }}
                          >
                            {checkoutContent?.deliveryInfo.meetingPoint.title ||
                              "Retiro en Punto de Encuentro"}
                          </h3>
                          <p
                            className="text-xs sm:text-sm mt-1"
                            style={{ color: !wantsShipping ? "#fff" : "#444" }}
                          >
                            {checkoutContent?.deliveryInfo.meetingPoint
                              .description ||
                  ...
                  >
                    <div className="flex items-start sm:items-center">
                      ...
                      <div className="flex items-start sm:items-center">
                        <Truck
                         className={`h-5 w-5 sm:h-6 sm:w-6 mr-2 sm:mr-3 flex-shrink-0 mt-1 sm:mt-0 ${
                          wantsShipping ? "text-white" : "text-black"
                        }`}
                        />
                        <div>
                          <h3
                            className="font-medium text-sm sm:text-base "
                            style={{ color: wantsShipping ? "#fff" : "#111" }}
                          >
                            {checkoutContent?.shipping.homeDelivery.title ||
                              "Envío a Domicilio"}
                          </h3>
                          <p
                            className="text-xs sm:text-sm mt-1"
                            style={{ color: wantsShipping ? "#fff" : "#444" }}
                          >
                            {checkoutContent?.shipping.homeDelivery
                              .description ||
                ...
                <div className="space-y-3 sm:space-y-4">
                    ...
                    >
                      <div className="flex items-start sm:items-center">
                        <div
                          ...
                          style={{
                            borderColor:
                              paymentMethod === "cash" ? "#fff" : "#000",
                            backgroundColor:
                              paymentMethod === "cash" ? "#fff" : "#fff",
                          }}
                        >
                          {paymentMethod === "cash" && (
                            <Check className="h-2 w-2 sm:h-3 sm:w-3 text-black" />
                        ...
                        <div className="flex items-start sm:items-center">
                          ...
                          <div>
                            <h3
                              ...
                              style={{
                                color:
                                  paymentMethod === "cash" ? "#fff" : "#111",
                              }}
                            >
                              {checkoutContent?.paymentInfo.cashOnDelivery
                  ...
                  {checkoutContent?.paymentInfo.bankTransfer.enabled && (
                    <div
                      ...
                      style={{
                        borderColor:
                          paymentMethod === "transfer" ? "#000" : "#e5e7eb",
                        background:
                          paymentMethod === "transfer"
                            ? "linear-gradient(90deg, #333 0%, #000 100%)"
                            : "#fff",
                    ...
                    >
                      <div className="flex items-start sm:items-center">
                        <div
                          ...
                          style={{
                            borderColor:
                              paymentMethod === "transfer" ? "#fff" : "#000",
                            backgroundColor:
                              paymentMethod === "transfer" ? "#fff" : "#fff",
                          }}
                        >
                          {paymentMethod === "transfer" && (
                            <Check className="h-2 w-2 sm:h-3 sm:w-3 text-black" />
                        ...
                        <div className="flex items-start sm:items-center">
                          ...
                          <div>
                            ...
                            >
                              {checkoutContent?.paymentInfo.bankTransfer
                ...
                checkoutContent?.paymentInfo.bankTransfer && (
                  ...
                  >
                    <h4
                      ...
                      style={{ color: "var(--deep-clay)" }}
                    >
                      {checkoutContent.paymentInfo.bankTransfer.title ||
                    ...
                    >
                      <p>
                        <strong>Banco:</strong>{" "}
                        {checkoutContent.paymentInfo.bankTransfer.bankName}
                      </p>
                      <p>
                        <strong>CBU:</strong>{" "}
                        {checkoutContent.paymentInfo.bankTransfer.cbu}
                      </p>
                      <p>
                        <strong>Alias:</strong>{" "}
                        {checkoutContent.paymentInfo.bankTransfer.alias}
                      </p>
                      <p>
                        <strong>Titular:</strong>{" "}
                        {checkoutContent.paymentInfo.bankTransfer.accountHolder}
                      </p>
          ...
          <div className="lg:col-span-4">
            <div className="bg-white rounded-2xl shadow-warm shadow-lg shadow-2xl lg:sticky lg:top-8">
              <div
                className="p-4 sm:p-6 border-b"
                style={{ borderColor: "var(--bone)" }}
              >
                <h2
                  className="font-serif text-lg sm:text-2xl font-light"
                  style={{ color: "var(--deep-clay)" }}
                >
                  Resumen de Orden
                </h2>
              ...
              <div className="p-4 sm:p-6">
                  ...
                  {items.map((item) => (
                    ...
                    >
                      ...
                      >
                        <img
                          src={
                            item.imagen || "/placeholder.svg?height=64&width=64"
                          }
                          alt={item.nombre}
                      ...
                      <div className="ml-3 sm:ml-4 flex-1 min-w-0">
                        ...
                        <p className="text-xs" style={{ color: "var(--oak)" }}>
                          {item.volumen.ml && `Volumen: ${item.volumen.ml}ml`}
                          {item.tipo && item.volumen.ml && " | "}
                ...
                <div
                  className="space-y-3 sm:space-y-4 border-t pt-4 sm:pt-6"
                  style={{ borderColor: "var(--bone)" }}
                >
                ...
                <p
                  className="text-xs text-center mt-3 sm:mt-4"
                  style={{ color: "var(--oak)" }}
                >
